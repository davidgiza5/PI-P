...
Blob::Blob(std::vector<cv::Point> _contur) {

    contur_curent = _contur;

    currentBoundingRect = cv::boundingRect(contur_curent);

    cv::Point centru;

    centru.x = (currentBoundingRect.x + currentBoundingRect.x + currentBoundingRect.width) / 2;
    centru.y = (currentBoundingRect.y + currentBoundingRect.y + currentBoundingRect.height) / 2;

    centerPositions.push_back(centru);
